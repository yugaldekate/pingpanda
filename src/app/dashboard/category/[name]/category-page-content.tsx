...
export const CategoryPageContent = ({ hasEvents: initialHasEvents, category }: CategoryPageContentProps) => {
    ...
    //setting the initial active tab based on the search params
    ...
    /*
        If pollingData.hasEvents === true → queryFn will run (or fetch from cache).
        Or if queryKey changes, it will also run.
    */
    ...
    const numericFieldSums = useMemo(() => {
        ...
        const now = new Date();
        const weekStart = startOfWeek(now, { weekStartsOn: 1 }); // Monday as the start of the week
            ...
            return (
                <Card key={field}>
                    <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <BarChart className="size-4 text-brand-700" />
                    </div>
    ...
    /* 
        ...
        Using useMemo to avoid unnecessary recalculations on every render
    */
    const columns: ColumnDef<Event>[] = useMemo(() => {
        return [
            ...
            ...(data?.events[0] ?
                // Dynamically create columns based on the fields in the first event
                Object.keys(data.events[0].fields as object).map((field) => ({
                        accessorFn: (row: Event) =>
                            (row.fields as Record<string, any>)[field], // typecast to Record<string, any>
                            header: field,
    ...
    // If there are no events in the category, show the empty state and try to refetch the events after every second
